...
#fileERT='CER2017-08-24_rhoa_notopo_geom.dat'
#fileRST='CER2017-08-24_tt_notopo_geom.dat'
#fileMesh='meshParaDomainCER.bms'
...
# load mesh previously created with BERT
mesh=pg.load(fileMesh)
...
err=ert.estimateError(ertData, absoluteError=0.01, relativeError=0.1)

resinv = ert.invert(ertData, mesh=mesh, err=err, lam=25)#, zWeight=0.1)
...
#ert.saveResult(cMin=500,cMax=3000,logScale=True)
ert.showData()
ert.showModel()
...
err=rst.estimateError(ttData, absoluteError=0.01, relativeError=0.01)

rst.inv.setMaxIter(20)
velinv = rst.invert(ttData, zWeight=1, lam=5, useGradient=True, vtop=300, vbottom=6000)
...
########### CONVENTIONAL 4PM ##########################################
fpm = FourPhaseModel()
fae, fie, fwe, maske = fpm.all(resinv, velinv)
np.savez("conventional.npz", vel=velinv, rho=resinv, fa=fae, fi=fie, fw=fwe)
...
##fpm = FourPhaseModel()
##
##rholayers = np.array([0, 400, 150, 150, 150])  # 0 is a dummy value
##vellayers = np.array([0, 800, 1500, 2000, 2800])  # 0 is a dummy value
##
