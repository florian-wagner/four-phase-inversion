...
# pg.show(meshERT, res)
ertData = ert.simulate(meshERT, res, ertScheme, noiseLevel=0.0, noiseAbs=0.0)
ert.setData(ertData)
ert.setMesh(meshERT)
ert.inv.setData(ertData("rhoa"))
ert.fop.regionManager().setZWeight(0.5)
resinv = ert.invert(ertData, mesh=meshERT, lam=20, zWeight=0.5)
...
rst.useFMM(False)
ttData = rst.simulate(meshERT, 1. / vel, ttScheme, noiseLevel=0.001)# noiseAbs=0.001)
