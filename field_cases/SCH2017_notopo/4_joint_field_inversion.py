...
ertScheme = pg.DataContainerERT("ertData.dat")
...
fwconventional = np.load("conventional.npz")["fw"]
fwconventional = np.load("conventional.npz")["fi"]
faconventional = np.load("conventional.npz")["fa"]
phi = 1 - pg.interpolate(mesh, fwconventional, meshRST.cellCenters()).array() - pg.interpolate(mesh, ficonventional, meshRST.cellCenters()).array() - pg.interpolate(mesh, faconventional, meshRST.cellCenters()).array()
